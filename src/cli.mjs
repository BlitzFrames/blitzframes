...
import {remotionFrom, inspectProject} from './project.mjs';
...
try {
  ...
  if (command === 'benchmark') {
    ...
    if (!project.ready) { console.error('Not a ready Remotion project: ' + project.reason); process.exit(1); }
    const remotion = await remotionFrom();
    ...
    await withBenchmarkFunctions({token: t, region: region(), projectDir: process.cwd(), spin}, async deployed => {
      ...
      const {summary, results} = await benchmark({projectDir: process.cwd(), region: region(), token: t, serveUrl, composition, inputProps, stockFunction: deployed.stockFunctionName,
        bfFunction: deployed.functionName, bfMemoryMb: deployed.memorySizeInMb, diskMb: remotion.constants.DEFAULT_EPHEMERAL_STORAGE_IN_MB, log, spin}, {remotion});
...
} catch (error) { console.error(process.env.BLITZFRAMES_DEBUG ? error.stack : error.message); process.exit(1); }
