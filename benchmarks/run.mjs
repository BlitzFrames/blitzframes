 ...
 * compositions, each wrapped in a Loop so that every length exists, at 100, 200 and 400 frames.
 *
...
import {PRICE_PER_FRAME} from '../src/benchmark.mjs';
...
const SIZES = (process.env.SIZES ?? '100,200,400').split(',').map(Number);
...
const summary = {measuredAt: new Date().toISOString(), completedAt: null, version: VERSION, region, pricePerFrame: PRICE_PER_FRAME, rows: []};
  ...
  for (const frames of SIZES) {
    ...
    if (result.status === 0 && existsSync(file)) {
      ...
      Object.assign(row, {chunks: s.chunks, width: s.dimensions?.width, height: s.dimensions?.height,
        stockMs: s.stock.warmMs, blitzMs: s.bf.warmMs, stockCost: s.stock.costUsd, blitzCost: s.bf.costUsd});
    } else {
